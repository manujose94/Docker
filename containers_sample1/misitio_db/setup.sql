...
INSERT INTO `robot_model` VALUES (2,'SUMMIT BASE',NULL,'4 ruedas','Robotnik','summit','R0b0tn1K','1'),(3,'RB1 BASE',NULL,'base rb1','Robotnik','rb1','R0b0tn1K','1'),(4,'RB1 SIMULado',NULL,'simulacion','Robotnik',NULL,NULL,'1');
...
INSERT INTO `robot_model_test` VALUES (2,3),(3,3),(3,5),(3,6),(3,7),(3,8),(3,9),(3,10),(3,11),(3,13);
...
INSERT INTO `robots_unit` VALUES (1,'summit_base','todoterreno de 4 ruedas','',2,'192.168.1.13700990'),(2,'rb1_base','brazo y base negra','158.42.165.114',3,'192.168.1.49'),(3,'rb1_base2','RB1','158.42.165.125',3,'192.168.1.59'),(4,'rb1_base','simulacion rb1',NULL,4,NULL);
...
INSERT INTO `test` VALUES (3,'Test Versión ROS y Sistema',' Verify which Ubuntu, Linux Kernel and ROS versions are installed','pythoncommands','Debe mostrar salida exitosa','auto',''),(5,'Test de velocidad angular','Command the robot at constant speed. Compare the velocity command published in the topic /robot/robotnik_base_control/cmd_vel/angular/z with the velocity published by the controller /robot/robotnik_base_control/odom_twist/twist/angular/z. Both velocities should be the same','python','','auto','rosrun test_rb1 Angular_Velocity_Check.py'),(6,'Test de batería','Comprobar estado bateria','python','','semi','rosrun test_rb1 Safety_Relay_Check.py'),(7,'Test de la IMU','Pixhawk ROS driver Check','python','','auto','rosrun test_rb1 Pixhawk_ROS_driver_Check.py'),(8,'Test Versión Librería PCAN',' Check the Peak-CAN version','commands','Comparar la versión con la predefinida, almacenar el resultado y dar NO OK en el caso de que sean diferentes','auto',NULL),(9,'Test de move','Send goal to /robot/move server to check if the robot moves the desired distance (This must be checked in X axis, Y axis if it’s an omni robot and Z rotation).','python',NULL,'semi','rosrun test_rb1 Robotnik_Move.py 1 0 0'),(10,'Test de parada de emergencia','If the robot has safety PLC, check if the software is correctly reading the different positions of the key. The values are published in /robot/safety_module/status\n1. If the safety is overridden, the safety_mode should be emergency\n2. If the safety is activated, the safety_mode should be safe','python',NULL,'semi','rosrun test_rb1 Safety_Relay_Check.py'),(11,'Test json','It need a params -j json','python',NULL,'auto','python3 test_json.py rb1_base'),(12,'Test sin modelo de robot','Consulta de información del servidor','commands','no se necesita robot','auto',NULL),(13,'Test socket','Comprobar funcionamiento de los socket con la interfaz','python','Solo para pruebas','semi','rosrun test_rb1 Socket_test.py');
...
-- Dump completed on 2020-04-27 15:39:55
