...
const qr = require('../db/querys_insert');
...
router.post('/start/:id', async (req, res) => {
    ...
    for (let i = 0; i < req.body.length - 1; i++) {
        if (req.body != null && req.body[i].type == "commands") {
 ...
 */
router.get('/startcommands/ssh', async (req, res) => {
    ...
    try {
        /**
         * ! Se debe de detectar si son enviados los parametros adecuados
         */
        ...
        console.log("[OBJECT RESULT] ", { test_model_unit });
        ...
        console.log("[Send responde]".green, output)
 ...
 */
router.get('/startcommands/local/', async (req, res) => {
    let commands = [];
