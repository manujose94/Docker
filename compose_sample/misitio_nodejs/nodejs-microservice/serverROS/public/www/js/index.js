...
function mSetMachineInfo() {
      ...
      params.set('com4', "hostname -I | cut -d' ' -f1");
      params.set('com5',"cat /sys/class/net/$(ip route show default | awk '"+'/default/'+"{print $5}')/address")
    
      getData(urls_get.start_commands + '?' + params.toString())
      .then((data) => {
        if (!data) return
        let { message, succes } = data;
        let innerHTML=""
        if (!succes)
          innerHTML = '<p class="text-warning"><strong>Warnning</strong></p>' + "Timeout to connect with robot model:" + model_id + '<p>' + message + '</p>';
        else{
          let array= message.split('::');
          console.log({array})

          for(let i in array)innerHTML=innerHTML+array[i]+'<br>';
         

        }
       
          document.getElementById("machine_local_info").innerHTML = '<i class="fa fa-desktop" aria-hidden="true"></i><strong>Detalles</strong><br>'+innerHTML;
      });
...
function mlistTopics(array, array2) {
  ...
  var searched = false;
  for (let i = 0; i < array.length; i++) {
    if (!searched) searched = array[i].includes("vrpn");
